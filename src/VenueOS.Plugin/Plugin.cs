...
public sealed class Plugin : IDalamudPlugin
{
    ...
    public Plugin()
    {
        ...
        // DJ Shouts: a specialized sibling of Greeter (NEW_MODULE_GUIDE.md's own framing) — purely local (no
        // backend, no shared/other-module state, and no dependency on Greeter's own runtime), reusing only the
        // shared SchedulerService/ChatCommandService/VenueProfileService every other timed-announcement module
        // already uses. UnderDevelopment/disabled by default (NEW_MODULE_GUIDE.md §22a) pending live QA.
        var djShoutsService = new VenueOS.Modules.Operations.DjShouts.DjShoutsService(scheduler, chat, venues, clock, diagnostics);
        var djShoutsPanel = new VenueOS.Plugin.DjShouts.DjShoutsOperatorPanel(djShoutsService, venues);
        var djShoutsModule = new VenueOS.Modules.Operations.DjShouts.DjShoutsModule(djShoutsService, djShoutsPanel.Draw, djShoutsPanel.DrawSettings);
